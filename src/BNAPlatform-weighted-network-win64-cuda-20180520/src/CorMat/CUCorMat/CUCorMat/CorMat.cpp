...
//					CorMat_cpu(Cormat_cpu, BOLD, N, L);
			...
			for (k = 0; k < NumS; k++)
			{
				...
				if (rs_flag){
					Clength = FormFullGraph(adjacent, Cormat_gpu, N, r_thresh[k]-ep);
	...
	if (fishtran_flag)
		for (int i = 0; i < N; i++)
			for (int j = i + 1; j < N; j++)
			{
				...
				if (Cormat_blocked[(long long)block_i*block_size + block_j + offset]>-1-ep && Cormat_blocked[(long long)block_i*block_size + block_j + offset]<1+ep)
