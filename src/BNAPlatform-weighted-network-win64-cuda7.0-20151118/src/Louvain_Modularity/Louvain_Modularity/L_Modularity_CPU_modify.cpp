...
double Louvain_modularity_sub1(  real_t* &W, R_type *R, C_type *C, V_type *V, long long *n1, int *Ci, long long N, double s)
{
	int i=0,j=0;
	long long n0=*n1;
	double q_gain = 0;
	...
	while (flag)
	{
		...
		{   i=a[idx];
			...
			for (R_type z = R[i]; z < R[i+1]; z++)
			{
				...
				dq += V[z];     //should be V[z] for weighted network. degree of i to M[C[j]];
				if(dq > max_dQ){
					max_dQ = dq;
					j = M[C[z]];
				}
...
double Louvain_modularity( int *Ci, R_type *R, C_type *C, V_type *V, long long N, int *Num_module)
{   
	double s = 0;
	double q_gain = 0;
	long long n0 = N;
	...
	i=1;
	real_t *W = NULL;
	q_gain = Louvain_modularity_sub1( W, R, C, V, &n0, Ci, N, s);  
	cout <<"Round "<<i<<endl;
	fout <<"Round "<<i++<<endl;
...
int main(int argc, char * argv[])
{
	...
	for (i = 0; i < FileNumber; i++)
	{
		...
		fin.read((char*)V, sizeof(V_type) * Vlength);
		...
		*/
		...
		V_type * V_dst = new V_type [Vlength];
		//Setup(0);
		//Start(0);
