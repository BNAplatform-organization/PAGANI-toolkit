...
//					CorMat_cpu(Cormat_cpu, BOLD, N, L);
			...
			a.append(filename[i]);
			OutCor = a.substr(0, a.find_last_of('.')).append("_").append(string(itoa(N, Graph_size, 10)));
			string Outfilename;
			ofstream fout;
		...
		if (argv[4][0] == 'y' || argv[4][0] == 'Y' || argv[4][0] == 'b' || argv[4][0] == 'B' )
		{
			...
			OutCor.append("ave_").append(string(itoa(FileNumber, Graph_size, 10))).append("_").append(string(itoa(N, Graph_size, 10)));
			...
			b = string(argv[1]).append("\\unweighted\\");
