...
__global__ void cuBFW_ph3(float * dst_ij, float * src_ik, float * src_kj);

void BFW_CUDA_ph3(float * dst_ij, float * src_ik, float * src_kj, int block_size)
{
	cudaFuncSetCacheConfig(cuBFW_ph3, cudaFuncCachePreferShared);
	dim3 dimBlock(16, 16); 
	...
	TimerInit(&tmr);
	TimerStart(&tmr);	
	cuBFW_ph3<<<dimGrid, dimBlock>>>(dst_ij, src_ik, src_kj);

/*	cudaThreadSynchronize();
	TimerStop(&tmr);
	double n1 = block_size / 1024.;
	double Gflop = 2*n1*n1*n1;
	cout<<"Phase 3 time: "<<TimerGetRuntime(&tmr)*1000<<"ms, Achieving: "<< Gflop / TimerGetRuntime(&tmr)<<"Gflop/s"<<endl;	
	*/
}
...
__global__ void cuBFW_ph3(float * dst_ij, float * src_ik, float * src_kj)
{
	...
	reg_dst_ij_00 = dst_ij[0 * DIM + 0];
	reg_dst_ij_01 = dst_ij[0 * DIM + 1];
	reg_dst_ij_02 = dst_ij[0 * DIM + 2];
	reg_dst_ij_03 = dst_ij[0 * DIM + 3];
	reg_dst_ij_04 = dst_ij[0 * DIM + 4];
	reg_dst_ij_05 = dst_ij[0 * DIM + 5];

	reg_dst_ij_10 = dst_ij[1 * DIM + 0];
	reg_dst_ij_11 = dst_ij[1 * DIM + 1];
	reg_dst_ij_12 = dst_ij[1 * DIM + 2];
	reg_dst_ij_13 = dst_ij[1 * DIM + 3];
	reg_dst_ij_14 = dst_ij[1 * DIM + 4];
	reg_dst_ij_15 = dst_ij[1 * DIM + 5];

	reg_dst_ij_20 = dst_ij[2 * DIM + 0];
	reg_dst_ij_21 = dst_ij[2 * DIM + 1];
	reg_dst_ij_22 = dst_ij[2 * DIM + 2];
	reg_dst_ij_23 = dst_ij[2 * DIM + 3];
	reg_dst_ij_24 = dst_ij[2 * DIM + 4];
	reg_dst_ij_25 = dst_ij[2 * DIM + 5];

	reg_dst_ij_30 = dst_ij[3 * DIM + 0];
	reg_dst_ij_31 = dst_ij[3 * DIM + 1];
	reg_dst_ij_32 = dst_ij[3 * DIM + 2];
	reg_dst_ij_33 = dst_ij[3 * DIM + 3];
	reg_dst_ij_34 = dst_ij[3 * DIM + 4];
	reg_dst_ij_35 = dst_ij[3 * DIM + 5];

	reg_dst_ij_40 = dst_ij[4 * DIM + 0];
	reg_dst_ij_41 = dst_ij[4 * DIM + 1];
	reg_dst_ij_42 = dst_ij[4 * DIM + 2];
	reg_dst_ij_43 = dst_ij[4 * DIM + 3];
	reg_dst_ij_44 = dst_ij[4 * DIM + 4];
	reg_dst_ij_45 = dst_ij[4 * DIM + 5];

	reg_dst_ij_50 = dst_ij[5 * DIM + 0];
	reg_dst_ij_51 = dst_ij[5 * DIM + 1];
	reg_dst_ij_52 = dst_ij[5 * DIM + 2];
	reg_dst_ij_53 = dst_ij[5 * DIM + 3];
	reg_dst_ij_54 = dst_ij[5 * DIM + 4];
	reg_dst_ij_55 = dst_ij[5 * DIM + 5];
