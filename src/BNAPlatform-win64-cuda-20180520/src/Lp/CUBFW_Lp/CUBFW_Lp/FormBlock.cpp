...
void init_block(float *costmat, const R_type *row, const C_type *col, const int numVertices, const int block_size)
{
	...
	{
		for ( long long j = row[i]; j < row[i+1]; j ++ ) 
		{
			int ab_row 		= i;
			int ab_col 		= col[j];
			...
			costmat[block_i * block_size + block_j + offset] = 1.0;
		}
	}

	for (long long k = 0; k < block_cnt; k++)
	{
		for (long long i = 0; i < block_size; i++)
		{
			costmat[(k * block_cnt + k) * block_size * block_size + i * block_size + i] = 0;
		}
	}
}
