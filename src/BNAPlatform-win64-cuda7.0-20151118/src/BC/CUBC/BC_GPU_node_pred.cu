...
__global__ void betweenness_node_pred_kernel(float *BC, int *r, int *c, int * dist, float * sigma, float * delta, int * P, int * P_end, int numVertices, int numEdges, int offset_source) 
{
...
void Betweenness_GPU_node_pred(int *r, int *c, int numVertices, int numEdges, float *BC, int grid, int thread)
{
	...
	{
		clock_t time = clock();
		betweenness_node_pred_kernel<<<grid, thread>>>(d_BC, d_r, d_c, dist, sigma, delta, P, P_end, numVertices, numEdges, offset_source);
