...
void Partition(long long N_C, int * R, int * C, int * Result);

//template <class Type> double VectorNorm(Type * x, bool * G, long long N);
void Maslov(int * R_dst, int * C_dst, int * R_src, int * C_src, unsigned int Rlength, unsigned int Clength);
...
int main(int argc, char * argv[])
{
	...
	for (i = 0; i < FileNumber; i++)
	{
		...
		/********************* Partition ******************/
		Partition(N_C, R_new, C_new, Result);
		/**************************************************/
		...
		for (long long l = 0; l < Maslov_num; l++)
		{
			...
			Partition(N_C, R_dst, C_dst, Result);
		}
		Stop(0);
