...
__global__ void standardAndThresholdingKernel(real__t* devCormat, int Batch_size, bool diagnoal, double thres)
{
...
/*************************************************************************************************/
int CorMat_gpu(string OutCor, real__t * BOLD_t, const int &N, const int &N0, const int &Num_Blocks, const int &L, const int &Batch_size, real__t* r_thresh, const int &NumS)
{			
	size_t L2 = L * ( L - 1 ) / 2;
	/*************************************************************************************************/
...
//#endif
	...
	{
		cout<<"error:"<<"R values abnormal(1)!"<<endl;
		return false;
	}
	//checking point!
	if (Row[N] != C.size() || Row[N] != V.size() )
	{
		cout<<"error:"<<"R values abnormal(2)!"<<endl;
		return false;
	}
