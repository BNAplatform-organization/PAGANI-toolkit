...
void CorMat_cpu(real__t * Cormat, real__t * BOLD, real__t *count_tie, int N, int L);
int CorMat_gpu(real__t * Cormat, real__t * BOLD,  int N, int L, int Batch_size, real__t * count_tie);
...
void post_block (real__t * Cormat, real__t * Cormat_blocked, int dim, int block_size,real__t * tie_count, int L2);
...
int main(int argc, char * argv[])
{
	...
	if (argc < 5) 
	{             //this declaration is of great essence!
	...
	cout<<argv[4][0]<<endl;
	if (argv[4][0] == 'r' || argv[4][0] == 'R' )
		for (i = 0; i < NumS; i++)
			r_thresh[i] = (real__t)atof(argv[5+i]);
	else if (argv[4][0] == 's' || argv[4][0] == 'S' )
	{
		rs_flag = false;
		memset(r_thresh, 0, sizeof(real__t)*NumS);
		for (i = 0; i < NumS; i++)
			s_thresh[i] = (real__t)atof(argv[5+i]);
	}
	else {
	...
	unsigned char *mask = new unsigned char[total_size];
	fin.read((char *) mask, sizeof(unsigned char) * total_size);
	...
	if (argv[3][0] == 'n' || argv[3][0] == 'N' || argv[3][0] == 'b' || argv[3][0] == 'B' )
		for (int i = 0; i < FileNumber; i++)
		{
			...
			// set some parameters 
			int Batch_size = 1024 ;				// should not be smaller than 1024 !
			...
			real__t * Count_tie_gpu = new real__t [N];
			memset((void*)Count_tie_gpu, 0, sizeof(real__t) * N);
			...
			//for(int k = 0; k<L*N; k++)
			//	BOLD[k] = (float) (rand()%25);
			
			// Begin computing correlation	
			time = clock();
			CorMat_gpu(Cormat_blocked, BOLD,  N, L, Batch_size, Count_tie_gpu);  //annotation:line 213 is core
			
			time = clock() - time;
			memset((void*)Cormat_gpu, 0, sizeof(real__t) * M1);
			post_block(Cormat_gpu, Cormat_blocked, N, Batch_size, Count_tie_gpu,L2);	

			cout<<"GPU correlation time: "<<time<<"ms"<<endl;
			//real__t * Cormat_cpu = new real__t [M1];
			//real__t * Count_tie_cpu = new real__t [N];
			//memset((void*)Count_tie_cpu, 0, sizeof(real__t) * N);
			//time = clock();
			//CorMat_cpu(Cormat_cpu, BOLD, Count_tie_cpu, N, L);
			//time = clock()-time;
			...
			for (int i = 0; i < N; i++)
			{
				...
				for (int j = i+1; j < N; j++)
				{
					if ((fabs(Cormat_gpu[kkk] - Cormat_cpu[kkk]) > 1e-6) || (Cormat_gpu[kkk] > 1))
					{
				...
				if (fabs(Count_tie_gpu[i] - Count_tie_cpu[i]) > 1e-6 )
				{
					cout<<"count_tie error"<<'\t'<<Count_tie_gpu[i]<<'\t'<<Count_tie_cpu[i]<<'\t'<<i<<endl;
					getchar();
				}
			}
			*/
	...
	if (argv[3][0] == 'y' || argv[3][0] == 'Y' || argv[3][0] == 'b' || argv[3][0] == 'B' )
		{
			...
			for (int i = 0; i < FileNumber; i++)
			{
				...
				time = clock();
			//	if (i == 0)
				{	   CorMat_gpu(Cormat_blocked, BOLD, N, L, Batch_size,Count_tie_gpu);
...
void post_block (real__t * Cormat, real__t * Cormat_blocked, int N, int block_size,real__t * tie_count, int L2)
{
/*	int i = 0;
	...
	*/
	...
	for (int i = 0; i < N; i++)
		for (int j = i + 1; j < N; j++)
		{
			int block_row = i / block_size;//1.joint one another to form upper triangle matrix
			...
			real__t temp = Cormat_blocked[(long long)block_i*block_size + block_j + offset];//cormat_gpu output is numerator
			temp /= sqrt((L2-tie_count[i])*(L2-tie_count[j])); //2.merge interface;gpu implement this step
			if (temp>=-1 && temp<=1)
			    Cormat[index] += temp;
...
{	
	...
	/*for (int i = 0; i < N; i++)
	{
		...
		for(int j =i+1; j < N; j++)
		{
			real__t *col = BOLD_t +j*L;
			float sumx = 0;
			for(int ii = 0; ii < L; ii++ )
				for(int jj = ii+1; jj < L; jj++)
					sumx +=((row[jj]>row[ii])-(row[jj]<row[ii]))*((col[jj]>col[ii])-(col[jj]<col[ii]));					
	...
	for (int i = 0; i < N; i++)
	{
		...
		for(int ii = 0; ii < L; ii++ )
			for(int jj = ii+1; jj < L; jj++)
				Count_tie[i] += real__t (row[jj]==row[ii]);
	}
		/*
