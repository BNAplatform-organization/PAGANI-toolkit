...
extern __global__ void standardAndThresholdingKernel(real__t* devCormat, int Batch_size, bool diagnoal, real__t thres);

extern __global__ void initialone(real__t *vec,int N);

extern void updateOperationii(real__t** operationii, real__t* devBOLDii, int L, int Batch_size, int ii);
extern void updateDevBOLDii(int ii, real__t* devBOLDii, real__t* BOLD_t, int L, int Batch_size, const int Num_Blocks);
extern void updatedevBOLDjj(int jj, real__t* devBOLDjj, real__t* BOLD_t, int L, int Batch_size, int* hosPoint, int* count, real__t* operationjj, const int Num_Blocks);
extern void updateOperationjj(real__t** operationjj, real__t* devBOLDjj, int L, int Batch_size, const int numjj);
...
/*************************************************************************************************/
int CorMat_gpu_blocking(string OutCor, real__t * BOLD_t, const int &N, const int &N0, const int &Num_Blocks, const int &L, const int &Batch_size,V_type *r_thresh, const int &NumS, const int Transferblocknum)
{			
	/*************************************************************************************************/
	...
	/******************   variables related to blocked transmission   ********************/
	...
	cout<<"blocked matrix correlating (blocked transmission)..."<<endl;
	...
	for (int ii = 0; ii < Num_Blocks; ii++)
	{
		...
		for (int jj = ii; jj < Num_Blocks; jj++)
		{
			...
			checkCublasErrors( cublasSgemm(handle, CUBLAS_OP_T,  CUBLAS_OP_N, Batch_size, Batch_size, L,  &alpha, operationii, L, operationjj, L, &beta, devCormat, Batch_size) );//virtually kernel
	...
	cout <<"\t      "<<"percent of memory in use: " << MemStat.dwMemoryLoad <<"%" <<"\t";
	cout << "free memory bytes: " << TOM(MemStat.dwAvailPhys) <<"M" <<endl;	
	
	long long M1 = (N-1);
	M1 *= N;	
	real__t spa = 100.0 * Row[N] / M1;
	cout<<"network info:"<<" ";
	cout<<"number of nonzero elements: "<<Row[N]<<endl;
	cout<<"\t      "<<"actual sparsity: "<<spa<<"%"<<"\t";
	cout<<"accurate generation mode:"<<" N/A"<<endl;
	char sparsity[30];
